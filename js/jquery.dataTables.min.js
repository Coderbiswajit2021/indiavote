...
oPaginate:{sFirst:"First",sLast:"Last",sNext:"Next",sPrevious:"Previous"},sEmptyTable:"No data available in table",sInfo:"Showing _START_ to _END_ of _TOTAL_ entries",sInfoEmpty:"Showing 0 to 0 of 0 entries",sInfoFiltered:"(filtered from _MAX_ total entries)",sInfoPostFix:"",sInfoThousands:",",sLengthMenu:"Show _MENU_ entries",sLoadingRecords:"Loading...",sProcessing:"Processing...",sSearch:"Search:",sUrl:"",sZeroRecords:"<span class='error01'>No matching records found</span>"},oSearch:h.extend({},j.models.oSearch),sAjaxDataProp:"aaData",
...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        var asInitVals, i, label, th, current_input;
        ...
        function fnCreateGtLtInput(oTable, regex, smart, bIsNumber, iFilterLength, iMaxLenght) {
            ...
            var input = $('<input type="text" class="' + search_init + sCSSClass + '" placeholder=" E.g. < 5"/>');
            if (iMaxLenght != undefined && iMaxLenght != -1) {
                input.attr('maxlength', iMaxLenght);
            }
            th.html(input);
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        ...
                        case "number":
                            ...
                            break;
                        case "GtLt":
                            fnCreateGtLtInput(oTable, true, false, true, aoColumn.iFilterLength, aoColumn.iMaxLenght);
                            break;
                        case "select":
...
/*  'unshift' does not work in IE6. A simply array concatenation would. This is used
...
function table2csv(oTable, exportmode, tableElm) {
    ...
    // Get header names
    $(tableElm+' thead').find('th:not([colspan]),[colspan="1"]').find('span').each(function() {
        var $th = $(this);
        var text = $th.text();
        var header = '"' + text + '"';
        tmpheaders.push(header); //KJ
        // headers.push(header); // original code
        if(text != "" && text != "#") headers.push(header); // actually datatables seems to copy my original headers so there ist an amount of TH cells which are empty
    });
    
    //console.log(((tmpheaders.length)/2)-1);
    if(tmpheaders[0] == '""'){ tmpheaders.splice(0, ((tmpheaders.length)/2)); }
    
    var lastEl = tmpheaders[tmpheaders.length-1];
    if(lastEl == '"ACs"') tmpheaders.splice(tmpheaders.length-1, 1); //KJ
   ...
   // if(headers[1] != '')
    ...
    if (exportmode == "full") { // total data
        ...
        for(i = 0; i < total; i++) {
          ...
          //  console.log(row);
            row = row.join(';'); //KJ
            row = strip_tags(row);
            row = row.replace( /,/g, "" ); //KJ
            //console.log(row);return false;
            row = row.replace( /;\s*AC View/g, "" ); //KJ
            row = row.replace( /;/g, "," ); //KJ
            rows.push(row);
        }
    ...
    $('body').append("<div class='csv-data'><form id='exportcsv' enctype='multipart/form-data' method='post' action="+base_url+"tools/export_csv><input name='pagetitle' type='hidden' value='"+ $('title').text() +"'><textarea class='form' style='display:none' name='csv'>"+csv+"</textarea></form></div>");
    $('#exportcsv').submit();
}
